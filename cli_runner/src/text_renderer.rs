...
impl TextRenderer {
    ...
    pub fn print<'a, FormatBuilder>(&self, text: &str, bounding_box: &Rect, format_option: FormatBuilder) -> Result<Path, RenderingError>
    where
        FormatBuilder: Into<TextFormatter<'a>>,
    {
        let format = format_option.into();
       let used_font = self.fonts.get(format.font).ok_or(RenderingError::ArgumentError)?;
       let used_font_face = used_font.font_faces.get(format.font_face).ok_or(RenderingError::ArgumentError)?;
       ...
       for c in text.chars() {
            match c {
                ...
                _ => {
                    match used_font.glyphs.get(c.borrow()) {
                        Some(glyph) => {
                            let glyph_path = glyph.path.borrow();
                            let current_line = line_buffer.last_mut().ok_or(RenderingError::DummyError)?;
                            let glyph_offset = Transform::translation(current_line.right_border, y_offset)
                                .then_scale(1.0, -1.0);
                            current_line.path = current_line.path.merge(glyph_path.transformed(glyph_offset.borrow()).borrow());
