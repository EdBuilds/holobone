...
impl TryFrom<Glyph> for LineGlyph {
    ...
    fn try_from(xml_glyph: Glyph) -> Result<Self, Self::Error> {
        let glyph_path = match xml_glyph.d {
            Some(svg_data) => build_path(
                SvgPathBuilder::new(lyon_path::Path::builder()),
                svg_data.as_ref(),
            )?,
...
pub fn load_font(file_path: &std::path::Path) -> Result<LineFont, RenderingError> {
    ...
    match parsing_result {
        Ok(parsed_xml_font) => {
            let default_horiz_adv: f32 =
                parsed_xml_font.defs.font.horizontal_advancement.parse()?;
            let missing_glyph_horiz_adv: f32 = parsed_xml_font
                .defs
                .font
                .missing_glyph
                .horizontal_advancement
                .parse()?;
            let mut parsed_line_font = LineFont {
                ...
                missing_glyph: MissingLineGlyph {
                    horizontal_advancement: missing_glyph_horiz_adv,
                },
                glyphs: HashMap::new(),
            };
            ...
            for font_face in parsed_xml_font.defs.font.font_faces {
                ...
                if let Ok(line_font_face) = maybe_line_font_face {
                    parsed_line_font
                        .font_faces
                        .insert(line_font_face.font_family.clone(), line_font_face);
                }
            }
            ...
            for glyph in parsed_xml_font.defs.font.glyphs {
                ...
                if let Ok(line_glyph) = maybe_line_glyph {
                    parsed_line_font
                        .glyphs
                        .insert(line_glyph.unicode_char, line_glyph);
                }
            }
