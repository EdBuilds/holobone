...
use crate::asteroids_game::physics::{MovementSimulator, RigidBody};
use crate::asteroids_game::pose::Pose;
use crate::asteroids_game::renderer::{render_entities, OutlineFactory, Renderable};
use crate::asteroids_game::shuttle::{
    Gun, Hierarchy, Score, ScoreUpdater, ShootingHandler, Shuttle, ShuttleUpdater,
};
use crate::renderer::Renderer;
use crate::renderer::RenderingError;
use crate::text_renderer::{TextAlignment, TextRenderer};
use euclid::vec2;
...
impl<'a, 'b> AsteroidsRenderer<'a, 'b> {
    pub fn new(
        text_renderer: &'a TextRenderer,
    ) -> Result<AsteroidsRenderer<'a, 'b>, RenderingError> {
        ...
        match gilrs {
            Ok(gilrs) => Result::Ok(AsteroidsRenderer {
                gilrs,
                active_gamepad: None,
    ...
    fn shuttle_update(
        ...
        match self.player_entity {
            Some(player_entity) => {
                ...
                gun.firing = firing;
                pose.rot -= Angle {
                    radians: (rot_input * rot_scaler),
                };
                if shuttle.is_dead() {
                    self.game_state = AsteroidsGameState::GameEnded;
                }
                Some(UITelemetry {
                    lives: shuttle.get_lives(),
    ...
    fn render_ui(&self, telemetry: UITelemetry) -> Result<Path, RenderingError> {
        ...
        let score_ui_path = self.text_renderer.print(
            telemetry.score.to_string().as_ref(),
            &score_text_box,
            TextAlignment::Left,
        )?;
        let lives_ui_path = self.text_renderer.print(
            telemetry.lives.to_string().as_ref(),
            &lives_text_box,
            TextAlignment::Right,
        )?;
    ...
    fn update_display(&mut self) -> Result<Path, RenderingError> {
        ...
        match self.game_state.borrow() {
            AsteroidsGameState::WaitForStart => {
                ...
                path = self
                    .text_renderer
                    .print("Press any key to start.", &wait_text_box, ())?;
                    ...
                    if let ButtonReleased(_, _) = event {
                        ...
                        self.player_entity = Some(
                            self.ecs
                                ...
                                .with(Shuttle::new())
                                .with(Renderable {
                                    outline: OutlineFactory::shuttle_outline(),
                                })
                                .with(Collider {
            ...
            AsteroidsGameState::Running => {
                ...
                if let Some(telemetry) =
                    self.shuttle_update(x_axis_input, y_axis_input, angle_input, firing_input)
                {
            ...
            AsteroidsGameState::GameEnded => {
                ...
                path = self.text_renderer.print(
                    ("Game over\nFinal score: ".to_owned() + &self.final_score.to_string())
                        .as_ref(),
                    &game_over_text_box,
                    (),
                )?;
